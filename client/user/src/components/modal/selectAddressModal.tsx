...
export default function SelectAddressModal({ address, updated }: { address: Array<Object>, updated: RefCallback<string> }) {
    const [open, setOpen] = useState(false);

    const defaultAddress = Array.isArray(address) && address.find(f => f['actived'])
    const [selected, setSelected] = useState(defaultAddress && defaultAddress['_id'].$oid);
    ...
    return (
        ...
        <div>
            ...
            <button
                ...
                className="block float-right me-5 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                Thay đổi
            </button>
            ...
            <div id="select-modal" className="overflow-y-auto overflow-x-hidden top-0 right-0  justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div className="absolute z-[10000] right-16 p-4 w-[30rem] max-h-full">
                    {/* <!-- Modal content --> */}
                    <div className="relative bg-blue-50 rounded-lg shadow dark:bg-gray-700">
                        ...
                        {/* <!-- Modal body --> */}
                            <div className="p-4 md:p-5">
                                <p className="text-gray-500 dark:text-gray-400 mb-4">Lựa chọn địa chỉ nhận hàng:</p>
                                            ...
                                            <label htmlFor={item['_id']?.$oid}
                                                ...
                                                <div className="w-full">
                                                        ...
                                                        <div className="flex justify-between">
                                                            <div>
                                                                <p className="text-gray-600 dark:text-white">{item['address']}</p>
                                                                <p className="text-green-500 dark:text-white">{item['actived'] ? 'Mặc định' : ''}</p>
                                                            </div>
