...
import { getFoodApi, getTopic } from "../../api/foodApi";

export default function Trend() {
    const { data: topic } = useQuery('topic', () => getTopic());
    const { data: food } = useQuery('foodList', () => getFoodApi());

    const newFood = Array.isArray(food) && food.length > 0 && food.sort((a: Object, b: Object) => b['updatedAt'] - a['updatedAt'])
    const bestSeller = Array.isArray(food) && food.length > 0 && food.sort((a: Object, b: Object) => b['sold'] - a['sold'])
    return (
        topic && newFood && bestSeller &&
        <div className="lg:mx-20 mx-10">
            ...
            <div className="mb-6">
                ...
                <div className="ms-5 flex flex-wrap ">
                    {newFood.slice(0,10).map((item, i) => (
            ...
            <div className="mb-6">
                ...
                <div className="ms-5 flex flex-wrap">
                    ...
                    {bestSeller.slice(0,10).map((item, i) => (
                        <FoodCard key={i} food={item} />
                    ))}
                </div>
            </div>
